...
export function CollectionView({ onOpenToday, onEditDay }: { onOpenToday: (day: number) => void; onEditDay: (day: number) => void }) {
  ...
  return <article className="paper-panel p-6 md:p-9">
    ...
    <div className="mt-7 grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-4 xl:grid-cols-5">{archive.map((symbol, i) => <button key={symbol} onClick={() => (i === selectedDay ? onOpenToday(i) : setSelectedDay(i))} className="text-left"><MiniBox index={i} symbol={symbol} active={i === selectedDay} large /></button>)}</div>
