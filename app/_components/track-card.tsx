...
export function TrackCard({ track, index, dimmed, date = "07 SEP" }: { track: Track; index: number; dimmed?: boolean; date?: string }) {
  return (
    <div className={`flex h-full flex-col overflow-hidden border border-black/15 bg-[#f3f4ef] p-5 text-left shadow-[0_18px_44px_rgba(18,22,20,.18)] md:p-6 ${dimmed ? "blur-[1px]" : ""}`}>
      <div className="flex items-start justify-between"><span className="brand text-5xl text-[#f5402a] md:text-6xl">{String(index + 1).padStart(2, "0")}</span><div className="text-right"><p className="font-bold">낯선 장르 {index + 1}</p><p className="text-sm text-black/60">의외의 발견</p></div></div>
