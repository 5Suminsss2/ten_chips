...
## 매일 자동 트랙리스트 초안 (예약 에이전트)

매일 00:00 KST 에 Claude 예약 에이전트가 그날 상자가 비어 있으면 트렌드/숨은 명곡/장르 중
하나를 골라 **초안(비공개)** 으로 대신 등록합니다. 사람이 검토·승인해야 실제로 공개됩니다.

**매일(또는 며칠에 한 번) 할 일** — ADMIN 화면에서:

1. 헤더 자물쇠 → 로그인 → 날짜 스트립에서 **"초안 · 검토 필요"**(주황색)로 표시된 오늘 날짜 확인.
2. 클릭해서 곡 목록(제목·아티스트·설명·태그·유튜브 영상) 검토.
3. 마음에 들면 그대로(또는 살짝 고쳐서) **저장** → 이 순간 공개됨.
4. 마음에 안 들면 **이 날짜 비우기**로 삭제(또는 직접 곡을 새로 입력).

아무것도 안 하면 초안 상태로 남을 뿐 공개되지 않으니, 안 볼 날엔 그냥 넘어가도 안전합니다.

**한 번만 하는 설정**(운영 서버에 `AUTOMATION_TOKEN` 등록)과 API 상세는
**[`server/DEPLOY.md` § 5](server/DEPLOY.md#5-매일-자동-초안-등록)** 참고.
예약 자체(시간 변경·일시정지·실행 로그)는 https://claude.ai/code/routines 에서 관리합니다.
